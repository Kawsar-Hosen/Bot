...
const verifyInstagramSession = async () => {
  try {
    ...
    state.me = null;
    state.lastError = { message: error.message, time: new Date().toISOString() };
    throw error;
  }
};
...
const fetchInboxPreview = async () => {
  ...
  try {
    ...
    state.lastInboxSync = new Date().toISOString();
    state.lastError = null;
  } catch (error) {
    state.lastError = { message: `Inbox sync: ${error.message}`, time: new Date().toISOString() };
...
app.get('/health', async (_req, res) => {
  ...
  if (state.loggedIn) {
    try {
      ...
      igStatus = 'session_expired';
      igError = err.message;
      state.loggedIn = false;
      state.lastError = { message: `Health check: ${err.message}`, time: new Date().toISOString() };
      addLog('error', `Health check detected expired session: ${err.message}`);
    }
  }

  res.json({
    status: igStatus === 'connected' || igStatus === 'not_logged_in' ? 'healthy' : 'degraded',
    uptime: {
    ...
    instagram: {
      status: igStatus,
      username: state.me?.username || null,
      loggedInSince: state.loginTime || null,
      error: igError,
    },
    bot: {
      running: state.running,
      lastInboxSync: state.lastInboxSync || null,
      pollerActive: !!state.poller,
    },
    memory: {
...
app.get('/diagnostics', async (_req, res) => {
  ...
  // 1. Server check
  checks.push({ name: 'Server', status: 'ok', detail: `Port ${port}, uptime ${Math.floor(process.uptime())}s` });

  // 2. Instagram session check
  if (state.loggedIn) {
    try {
      const user = await ig.account.currentUser();
      checks.push({ name: 'Instagram Session', status: 'ok', detail: `@${user.username} active` });
    } catch (err) {
      checks.push({ name: 'Instagram Session', status: 'fail', detail: err.message });
      state.loggedIn = false;
      state.lastError = { message: err.message, time: new Date().toISOString() };
    }
  } else {
    checks.push({ name: 'Instagram Session', status: 'warn', detail: 'Not logged in' });
  }

  // 3. Inbox access check
  if (state.loggedIn) {
    try {
      const feed = ig.feed.directInbox();
      await feed.items();
      checks.push({ name: 'Inbox Access', status: 'ok', detail: 'Inbox readable' });
    } catch (err) {
      checks.push({ name: 'Inbox Access', status: 'fail', detail: err.message });
    }
  } else {
    checks.push({ name: 'Inbox Access', status: 'skip', detail: 'Requires login' });
  }
  ...
  checks.push({
    ...
    detail: state.running ? `Running, poller ${state.poller ? 'active' : 'inactive'}` : 'Stopped',
  });
  ...
  checks.push({
    name: 'Memory',
    status: heapMB > 450 ? 'warn' : 'ok',
    detail: `Heap ${heapMB.toFixed(1)} MB / RSS ${(mem.rss / 1024 / 1024).toFixed(1)} MB`,
  });
  ...
  checks.push({
    name: 'Log Buffer',
    status: 'ok',
    detail: `${state.logs.length} entries (max 300)`,
  });
